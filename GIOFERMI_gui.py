...
def LC_func(entry_mail, entry_password, entry_LCname):
	...
	if ans == 1:
		...
		try:
			...
			messagebox.showerror('Error', 'Proccess done, but invalid email address. Unable to send message')
	else:
		label_results['text'] = 'Proccess canceled. Copy to run: . %s_LC_commands.txt' % (entry_LCname)
...
def setup_func(entry_mail, entry_password):
	...
	try:
		...
		messagebox.showerror('Error', 'Invalid email adress')
